...
static func get_types() -> Array:
	var arr = .get_types()
	arr.append("CrossFadeTransition")
	return arr
...
# node_holder should have a material with a tint_color.shader attached
onready var node_holder: Node2D = get_node(node_holder_path)
...
func _process(delta):
	if time < duration:
		time += delta
		node_holder.modulate = fade_curve.interpolate(time / duration) 
	else:
		set_process(false)
