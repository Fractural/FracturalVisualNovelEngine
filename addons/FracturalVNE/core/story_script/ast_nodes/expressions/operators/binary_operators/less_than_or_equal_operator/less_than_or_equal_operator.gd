...
func evaluate():
	...
	if not SSUtils.is_success(left_result):
		return stack_error(left_result, "Could not perform a less than or equal to comparison.")
	var right_result = SSUtils.evaluate_and_cast(right_operand, "Number")
	if not SSUtils.is_success(right_result):
		return stack_error(right_result, "Could not perform a less than or equal to comparison.")
