...
const EOF = "EOF"
# TODO: Add custom implmementation for expect("literal")
...
func expect_string():
	if reader.peek().type == TT_STRING:
		return reader.consume()
	return error('Expected a string.')

func expect_integer():
	if reader.peek().type == TT_INT:
		return reader.consume()
...
func expect_float():
	if reader.peek().type == TT_FLOAT:
		return reader.consume()
	return error('Expected a float.')

func expect_decimal():
	if reader.peek().type == TT_FLOAT or reader.peek.type == TT_INT:
		return reader.consume()
	return error('Expected a decimal number.')
...
func expect_say_statement():
	var character_name = expect_string()
	
	# Try to parse character variable if no character name is provided
	if not is_success(character_name):
		character_name = expect_identifier()
	
	if is_success(character_name):
		var message = expect_string()
		if is_success(message):
			var say_statement = Statement.new()
			say_statement.kind = StatementKind.FUNCTION_CALL;
			say_statement.name = "say"
			# TODO
		else:
			return message
	else:
		# Try parse only message (narration) 
		var message = expect_string()
		if is_success(message):
			pass
			# TODO
