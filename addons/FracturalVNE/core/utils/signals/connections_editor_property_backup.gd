...
var node_path_arr_editor_prop = null
...
func get_node_path_editor_prop(index):
	
	var result = node_path_arr_editor_prop.get_node("VBoxContainer").get_child(2 + index).get_node("EditorPropertyNodePath")
	return result


func get_node_path_editor_prop_assign_button(index):
	return get_node_path_editor_prop(index).get_node("HBoxContainer/Button")


func show_node_path_selector(index):
	print("Showing Path Selector")
	var assign_node_path_button = get_node_path_editor_prop_assign_button(index)
	var editor_prop = get_node_path_editor_prop(index)
	if not "_on_node_selected" in editor_prop.get_signal_connection_list("property_changed"):
		editor_prop.connect("property_changed", self, "_on_node_selected", [index])
	if assign_node_path_button != null:
		assign_node_path_button.emit_signal("pressed")


func update_property():
	if node_path_arr_editor_prop == null:
		return
	...
	# ----- Resizing Dummy Array Size ----- #
	
	var listener_paths_size = get_edited_object()[get_edited_property()].size()
	var dummy_paths_size = node_path_arr_editor_prop.get_edited_object()[node_path_arr_editor_prop.get_edited_property()].size()
	print("RESIZING " + str(listener_paths_size) + " vs " + str(dummy_paths_size))
	
	if dummy_paths_size != listener_paths_size:
		print("RESIZING TO " + str(listener_paths_size))
		var arr = []
		for connection in get_edited_object()[get_edited_property()]:
			arr.append(connection.listener_path)
		arr.resize(listener_paths_size)
		
		node_path_arr_editor_prop.get_edited_object()[node_path_arr_editor_prop.get_edited_property()] = arr
		node_path_arr_editor_prop.update_property()
	#	node_path_arr_editor_prop.emit_changed(node_path_arr_editor_prop.get_edited_property(), arr)
		
		#yield(node_path_arr_editor_prop, "property_changed")
...
func set_connection(connection, index):
	...
	if not connection.listener_path.is_empty():
		...
		var node_path_editor_prop = get_node_path_editor_prop(index)
		#node_path_editor_prop.emit_changed(node_path_editor_prop.get_edited_property(), connection.listener_path)
		#assign_button.icon = get_node_path_editor_prop_assign_button(index).icon
		print("ASD " + str(node_path_editor_prop) + "icon: " + str(get_node_path_editor_prop_assign_button(index).icon))
	else:
...
func _on_node_selected(property, selected_node_path, field, changing, index):
	if updating:
		return
	
	print("THIS IS CURSED! Selected NodePath: " + str(selected_node_path))
	_on_connection_listener_path_changed(selected_node_path, index)


func _hijack_dummy_node_path_editor_prop():
	print("Jacking in")
	
	node_path_arr_editor_prop = get_node("../EditorPropertyArray")
...
#	cancel_button.connect("pressed", self, "_on_node_selection_closed")
	...
	#node_path_arr_editor_prop.visible = false
