...
onready var story_state_manager = StoryServiceRegistry.get_service("StoryStateManager")
...
func _on_state_saved(state, slot_id):
	save_slots_menu.ui_save_slots[slot_id].construct(slot_id, story_state_manager.story_save_manager.save_slots[slot_id])
