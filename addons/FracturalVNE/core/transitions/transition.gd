...
const MergedReplaceTransition = preload("res://addons/FracturalVNE/core/transitions/merged_replace_transition.gd")
...
func _ready():
	if replace_transition == null:
		replace_transition = MergedReplaceTransition.new()
		replace_transition.init(show_transition, hide_transition)
...
func hide_transition(old_node_: Node2D, duration_: float):
	...
	hide_transition.connect("transition_finished", self, "_on_transition_finished")
...
func replace_transition(new_node_: Node2D, old_node_: Node2D, duration_: float):
	...
	replace_transition.connect("transition_finished", self, "_on_transition_finished")
...
func _on_transition_finished(skipped):
	...
	show_transition.disconnect("transition_finished", self, "")
